...
mu_vec = [-1.0, -0.5, -0.15, 0.0, 0.15, 0.5, 1.0]


model_num="11"

args=[("c2",[1,2]),("c4",0),("st",[1,2,3,4]),("x0",[1]),("bias_d",[1]),("hbias_d",0),("sigma_a",[1])]
consts=[("c2",0),("c4",[1]),("st",0),("x0",[2]),("bias_d",0),("hbias_d",[1]),("sigma_a",0)]

z = [1,0,0]

lb=[-80.,-80.,0.0,0.0,-80.0,-80.0,-80.,-80,0.1]
ub=[80.,80.,100.,100.,80.,80.,80.,80.,80.]
...
aux=2*(rand(9)-0.5)
x_ini=[ aux[1]*80.,aux[2]*80.,abs(aux[3]*100.),abs(aux[4]*100.),aux[5]*80,aux[6]*80,aux[7]*80,aux[8]*80,rand()*79.9+0.1]
if n_stim_10 != 0
    if n_stim_10 > 10000
        n_stim = 10000
    else
    ...
    ## SAVE THE RESULTS ##
    j=1
    path_save_res=path*"results/model13_"*dataset*"/Rat"*rat_num*"_10/"
...
if n_stim_20 != 0
    ...
    ## SAVE THE RESULTS ##
    j=1
    path_save_res=path*"results/model13_"*dataset*"/Rat"*rat_num*"_20/"
    mkpath(path_save_res)
    filename_save=path_save_res*"icond"*string(j)*".json"
