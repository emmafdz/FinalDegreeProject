...
aux=2*(rand(6)-0.5)
x_ini=[ aux[1]*20.,abs(aux[2]*20.),abs(aux[3]*20.),aux[4]*20.,aux[5]*20.,rand()*19.9+0.1]


if n_stim_10 != 0
    history_bias=zeros(n_stim_10)[15000:25000,:]
    Tframe = 0.05
    stim_10 = permutedims(reshape(stim_10,(10,n_stim_10)))[15000:25000,:]
    choices_10 = data["choices_10"][15000:25000,:]
    mu_10 = data["coherences_10"][15000:25000,:]
    sigma_10 = data["sigma_10"][15000:25000,:]
    fit = ModelFitting_rats(stim_10,sigma_10,mu_10,mu_vec,choices_10,history_bias, Tframe, args,consts,ub,lb, x_ini,z,model_f)
end
if n_stim_20 != 0
    history_bias=zeros(n_stim_20)[15000:25000,:]
    Tframe = 0.05
    stim_20 = permutedims(reshape(stim_20,(20,n_stim_20)))[15000:25000,:]
    choices_20 = data["choices_20"][15000:25000,:]
    mu_20 = data["coherences_20"][15000:25000,:]
    sigma_20 = data["sigma_20"][15000:25000,:]
...
## SAVE THE RESULTS ##
j=1
path_save_res=path*"results/fit_model12"*dataset*"/Rat"*rat_num*"/"
