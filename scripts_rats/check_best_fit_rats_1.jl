...
#    stim_rep=repeat(transpose(stim[1,:]),Ntrials)
    ...
    println("dt",dt)
    s_trans = zeros(10)
    simulations = zeros(51,100)
	for r in 1:100
		for iframe in 1:10
			s_trans[iframe]=stim_rep[iframe]*(param_best["st"][1]+param_best["st"][2]*(iframe-1))
		end
		println("s_trans_1",s_trans)
		simulation = zeros(51)
		...
		println("s",s)
			for j in 1:5
				mu = -s_trans[s]-param_best["c2"][1]*simulation[counter]+param_best["c4"][1]*simulation[counter]^3
				#mu = -s_trans[s]-param_best["c2"][1]*simulation[counter]*2+4*param_best["c4"][1]*simulation[counter]^3

				simulation[counter+1] = simulation[counter] - (dt/tau)*(-mu)+sqrt(dt/tau)*(param_best["sigma_a"][1]*randn())
			counter +=1
		    #d_1, xfinal=simulation_Nframe_general_trace(param_best,stim_rep,internal_noise_sigma,T,tau,model_f)
			end
		end
		simulations[:,r]=simulation
	end
    
    print("he fet això")


    save=Dict("sim"=>simulations)

    path_save=path*"figures/"

    json_string = json_string = JSON.json(save)
	open(path_save*"one_pdf_sim.json","w") do f
		write(f, json_string)
	print("done")
	end
