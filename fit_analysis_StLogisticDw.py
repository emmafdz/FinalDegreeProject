...
#for rat in ["rat24"]:
    # Opening JSON file 
    f = open(directory_results+'model_fitStLogTimeDw/rat'+rat+'_05sec.json',) 
...
for rat in ["rat25","rat35","rat37"]:
    ...
    # Opening JSON file 
    f = open(directory_results+'model_fitStLogTimeDw/'+rat+'_1sec.json',) 
    # returns JSON object as  
    # a dictionary 
    data = json.load(f) 
    data.pop("ci")
        ...
        for p in range(len(param_fits[rat][param])):
            if param_fits[rat][param][p]!=0:
                if param_fits[rat][param][p]!=1:
                    ...
                    ind +=1
    axs1.set_xticks([0,1,2,3,4])
    axs1.set_xticklabels(["$c_2$","A",r"$\beta$",r"$\beta_t$","$c_4$"],fontsize = 20)
    plt.yticks(fontsize = 20)
    fig.suptitle("Parameters fit", fontsize = 20)
    axs1.spines["top"].set_visible(False)
    axs1.spines["right"].set_visible(False)
    axs1.hlines(0,0,5,linestyle="dashed",color="grey")
    axs1.set_ylim([-0.5,12])
#     axs1.set_xticks([-0.2])
#     axs1.set_xlim([-0.1,1])
#     axs1.set_title("c2")
...
for rat in range(len(rats)):
    ...
    axs1.set_xticklabels(["$c_2$","A",r"$\beta$","$c_4$"],fontsize = 20)
    plt.yticks(fontsize = 20)
    fig.suptitle("Parameters fit", fontsize = 20)
    axs1.spines["top"].set_visible(False)
    axs1.spines["right"].set_visible(False)
...
for rat in range(len(rats)):
    ...
    for stim in stimulus[rat]:
        ...
        for iframe in range(len(stim)):
            s_trans.append((1.0/st1)*(1.0/(1+np.exp(-( (1./st2+st3*(iframe-1))*(stim[iframe] ) ) ) ) -0.5 ))
...
#         plt.ylim([-1,1])
    ...
    plt.yticks([-0.5,0,0.5],fontsize=22)
...
#     fig.suptitle(" Psychometric curve "+rats[rat],fontsize = 22)
    ...
    fig.text(0.5, -0.04, "Stimulus strength ($\mu$)", ha='center',fontsize = 28)
