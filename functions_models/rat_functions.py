...
def divide_coh(coh,reward,decision,performance,stim,target_sigma,coherence):
    ...
    a = 0
    for c in coh:
        indexes = np.where(np.asarray(c) == coherence)[0]
        for i in indexes:
            reward_1.append(reward[a][i])
            decision_1.append(decision[a][i])
            performance_1.append(performance[a][i])
            stim_1.append(stim[a][i])
            target_sigma_1.append(target_sigma[a][i])
        a +=1
...
def PK(decision,stim,axs,col):
    # provar sklearn
    result = LogisticRegression().fit(decision, stim)
...
def PK_1(decision,stim,axs,col):
    # provar sklearn
...
def divide_sigma_1(reward,decision,performance,stim,target_sigma,sigma):
    ...
    stim_1 = []
    a = 0
    for c in target_sigma:
        indexes = np.where(np.asarray(c) == sigma)[0]
        for i in indexes:
            reward_1.append(reward[a][i])
            decision_1.append(decision[a][i])
            performance_1.append(performance[a][i])
...
def transformed_stimulus(stimulus):
    ...
    for rat in range(len(stimulus)): # half_sec_stim_list_all or sec_stim_list_all
        transf_stim = []
        for sig in range(len(stimulus[rat])):
            t_stim = []
            for stim in stimulus[rat][sig]:
                stim_sorted = np.sort(stim)
                hist, base = np.histogram(stim,bins = 11,range = (-1,1))
                t_stim.append(hist)
            transf_stim.append(t_stim)
