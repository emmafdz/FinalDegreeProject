...
def get_color_subjects():
    ...
    return colors_subject

def get_param_labels(model,lam):

    if (model=="StLinearPi") or (model=="StLinearPi"+"Lambda"+str(lam) ):
        labels=[r"$\beta$"]
        nrow=1
        ncol=1
    elif (model=="StLogisticPi") or (model=="StLogisticPi"+"Lambda"+str(lam)):
        labels=[r"A",r"$\beta$"]
        nrow=1
        ncol=2
    elif (model=="StLogisticTimeDw") or (model=="StLogisticTimeDw"+"Lambda"+str(lam)):
        labels=[r"$c_2$",r"$c_4$",r"A",r"$\beta$",r"$\beta_t$"]
        nrow=2
        ncol=3
    elif (model=="StLinearTimeExpDw") or (model=="StLinearTimeExpDw"+"Lambda"+str(lam)):
        labels=[r"$c_2$",r"$c_4$",r"$\beta$",r"$\beta_t$"]
        nrow=4
        ncol=1

    elif (model=="StLinearDw") or (model=="StLinearDw"+"Lambda"+str(lam)):
        labels=[r"$c_2$",r"$c_4$",r"$\beta$"]
        nrow=1
        ncol=3

    elif (model=="StLogisticDw") or (model=="StLogisticDw"+"Lambda"+str(lam)):
        labels=[r"$c_2$",r"$c_4$",r"A",r"$\beta$"]
        nrow=1
        ncol=4
